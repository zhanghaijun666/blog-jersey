requirejs(["bcstore"], function (bcstore) {
    ...
    function RootViewModel() {
        ...
        self.blogNavigator = function () {
            let arr = [];
            arr.push({text: '我的博客', icon: 'fa-book', click: self.changeHash.bind(null, "home"), isSelect: ko.computed(function () {
                    return self.isHash("home");
                })});
            arr.push({text: '消息', icon: 'fa-comments-o', click: self.changeHash.bind(null, "messages"), isSelect: ko.computed(function () {
                    return self.isHash("messages");
                })});
            arr.push({text: '系统管理', icon: 'fa-coffee', click: self.changeHash.bind(null, "admin"), isSelect: ko.computed(function () {
                    return self.isHash("admin");
                })});
            if (self.isSmallScreen()) {
                arr.push({text: '我的', icon: 'fa-coffee', click: self.changeHash.bind(null, "admin"), isSelect: ko.computed(function () {
                        return self.isHash("admin");
                    })});
            }
